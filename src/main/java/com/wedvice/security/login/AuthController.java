...
public class AuthController {
    ...
    /**
     * 1. 로그인 성공 후 백엔드에서 프론트에 /Redirection으로 보낸다 2. 프론트에서는 그 후 /status로 요청을 보낸다 3. /status경로는
     * security filter에서 permitall . 로그인이 안되어있으면 isLoggedIn : false로 반환값이 간다. 4. 정상이면 여기서 화면 값을
     * 받아간다.
     **/
    @GetMapping("/status")
    @Operation(
    ...
    public ResponseEntity<ApiResponse<RedirectResponseDto>> getLoginStatus(
        ...
        RedirectResponseDto redirectResponseDto = userService.getRedirectStatus(
            loginUser.getUserId());
